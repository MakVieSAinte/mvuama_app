...
const getPaymentIcon = () => {
  switch (currentPlan.value.paymentMethod.type) {
    case 'card':
      return CreditCard
    case 'paypal':
      return CreditCard // Remplacer par l'icône PayPal si disponible
    case 'bank':
      return CreditCard // Remplacer par l'icône Bank si disponible
    default:
      return CreditCard
  }
}
...
<template>
  <Card>
    <CardHeader>
      ...
      <CardDescription> Détails et gestion de votre abonnement en cours </CardDescription>
    </CardHeader>
        ...
        <div class="flex items-start justify-between">
          <div>
            ...
            <p class="text-muted-foreground">
              {{ currentPlan.price }} € /
              {{ currentPlan.billingCycle === 'monthly' ? 'mois' : 'an' }}
            </p>
          </div>
    ...
    <CardFooter class="flex flex-wrap gap-2">
      ...
      <Button variant="outline" as="a" href="/subscription/available-plans">
        Changer de plan
      </Button>
      <Button
        variant="destructive"
  ...
  <Dialog v-model:open="showCancelDialog">
    <DialogContent>
      <DialogHeader>
        ...
        <DialogDescription>
          Vous conserverez l'accès à tous les services jusqu'à la fin de votre période de
          facturation actuelle, le {{ formatDate(currentPlan.renewalDate) }}.
        </DialogDescription>
      </DialogHeader>
      ...
      <DialogFooter class="mt-4">
        <Button variant="outline" @click="showCancelDialog = false"> Retour </Button>
